...
namespace finite_element::assembly::error_system
{
    ...
    public:
        ...
        static_assert(
            ...
            "SharedLocalErrorContext2D requires 2D space-time cells.");
            ...
            const ActiveSlabCellVector& active_slab_cells)
        {
            ...
            result.x_geometry_by_cell_id_.resize(
                static_cast<std::size_t>(x_space.mesh_ref().n_cells()));
            result.slab_geometry_by_ordinal_.reserve(
                active_slab_cells.size());
            result.slab_cell_metadata_.reserve(active_slab_cells.size());
            result.slab_cell_ordinal_by_key_.reserve(
                active_slab_cells.size());
            result.slab_cell_ordinal_by_dense_id_.resize(
            ...
            for (const auto& cell : active_slab_cells)
            {
                ...
                const int active_x_cell_id = ancestor_cache.find(source_cell_id);
                ...
                if (slab_id >= 0 &&
                    static_cast<std::size_t>(slab_id) <
                        result.slab_cell_ordinal_by_dense_id_.size() &&
                    slab_cell_id >= 0)
                {
                    auto& dense =
                        result.slab_cell_ordinal_by_dense_id_[
                            static_cast<std::size_t>(slab_id)];
                    if (static_cast<std::size_t>(slab_cell_id) >=
                        dense.size())
                    {
                        dense.resize(
                            static_cast<std::size_t>(slab_cell_id) + 1u,
                            unknown_cell_);
                    }
                    dense[static_cast<std::size_t>(slab_cell_id)] = ordinal;
                }
            }
        ...
        [[nodiscard]] std::size_t estimated_memory_bytes() const noexcept
        {
            return sizeof(*this) +
                   ...
                   slab_cell_metadata_.capacity() *
                       sizeof(SlabCellMetadata) +
                   slab_cell_ordinal_by_key_.size() *
                       (sizeof(std::pair<const std::uint64_t, int>) +
                        3u * sizeof(void*)) +
                   dense_ordinal_map_memory_bytes_();
        }
            ...
            int slab_cell_id) const
        {
            if (slab_id >= 0 &&
                static_cast<std::size_t>(slab_id) <
                    slab_cell_ordinal_by_dense_id_.size())
            {
                const auto& dense =
                    slab_cell_ordinal_by_dense_id_[
                        static_cast<std::size_t>(slab_id)];
                if (slab_cell_id >= 0 &&
                    static_cast<std::size_t>(slab_cell_id) < dense.size())
                {
                    const int ordinal =
                        dense[static_cast<std::size_t>(slab_cell_id)];
                    if (ordinal != unknown_cell_)
                        return ordinal;
                }
            }
            const auto it =
            ...
            if (it == slab_cell_ordinal_by_key_.end())
                throw std::runtime_error(
                    "SharedLocalErrorContext2D: slab cell not indexed.");
            return it->second;
        }
            ...
            int ordinal) const
        {
            ...
            {
                throw std::runtime_error(
                    "SharedLocalErrorContext2D: invalid slab-cell ordinal.");
            }
            ...
            int ordinal) const
        {
            ...
            {
                throw std::runtime_error(
                    "SharedLocalErrorContext2D: invalid slab geometry ordinal.");
            }
            ...
            int source_cell_id) const
        {
            ...
            {
                throw std::runtime_error(
                    "SharedLocalErrorContext2D: invalid source cell id.");
            }
            const int cell_id =
            ...
            if (cell_id == unknown_cell_)
                throw std::runtime_error(
                    "SharedLocalErrorContext2D: source cell not indexed.");
            return cell_id;
        }
        ...
        [[nodiscard]] const XGeometryData& x_geometry(int cell_id) const
        {
            ...
            {
                throw std::runtime_error(
                    "SharedLocalErrorContext2D: invalid X cell id.");
            }
            const auto& entry =
                x_geometry_by_cell_id_[static_cast<std::size_t>(cell_id)];
            if (!entry.has_value())
                throw std::runtime_error(
                    "SharedLocalErrorContext2D: X geometry not indexed.");
            return *entry;
        }
    ...
    private:
        ...
        void ensure_source_index_(int source_cell_id) const
        {
            ...
            {
                throw std::runtime_error(
                    "SharedLocalErrorContext2D: source cell id out of range.");
            }
        }
        ...
        void ensure_x_geometry_index_(int cell_id) const
        {
            ...
            {
                throw std::runtime_error(
                    "SharedLocalErrorContext2D: X cell id out of range.");
            }
        }
            ...
            const B& b) noexcept
        {
            ...
            update(a.map.space.v0[0], b.map.space.v0[0]);
            update(a.map.space.v0[1], b.map.space.v0[1]);
            update(a.map.space.v1[0], b.map.space.v1[0]);
            update(a.map.space.v1[1], b.map.space.v1[1]);
            update(a.map.space.v2[0], b.map.space.v2[0]);
            update(a.map.space.v2[1], b.map.space.v2[1]);
            ...
            update(a.J10, b.J10);
            update(a.J11, b.J11);
            update(a.invJ00, b.invJ00);
            update(a.invJ01, b.invJ01);
            update(a.invJ10, b.invJ10);
            update(a.invJ11, b.invJ11);
            update(a.invJT00, b.invJT00);
            update(a.invJT01, b.invJT01);
            update(a.invJT10, b.invJT10);
            update(a.invJT11, b.invJT11);
            update(a.t0, b.t0);
            update(a.ht, b.ht);
        ...
        [[nodiscard]] std::size_t dense_ordinal_map_memory_bytes_()
            const noexcept
        {
            std::size_t bytes =
                slab_cell_ordinal_by_dense_id_.capacity() *
                sizeof(std::vector<int>);
            for (const auto& per_slab : slab_cell_ordinal_by_dense_id_)
                bytes += per_slab.capacity() * sizeof(int);
            return bytes;
        }
        ...
        std::vector<std::vector<int>> slab_cell_ordinal_by_dense_id_{};
