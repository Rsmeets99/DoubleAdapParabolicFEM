...
namespace finite_element::error_fespace
{
    ...
    public:
        ...
        static_assert(GT::dim_space_v == 1,
                      "PatchScalarSpace requires a 1D spatial edge patch.");
        static_assert(GT::dim_time_v == 1,
                      "PatchScalarSpace requires dim_time_v == 1.");
        static_assert(PSpace >= 1,
                      "PatchScalarSpace requires PSpace >= 1.");
        ...
        explicit PatchScalarSpace(const PatchType& patch)
            : patch_(&patch),
              dof_map_(
                  patch.is_boundary() ? (PSpace + 1) : max_spatial_dofs_v,
                  n_time_dofs_v)
        {}
        ...
        void evaluate_spatial_basis_on_patch_cell(
            int patch_cell_index,
            double x_ref,
            SpatialValues& values) const
        {
            evaluate_spatial_on_cell_(patch_cell_index, x_ref, values);
        }
        ...
        void evaluate_on_cell(
            ...
            double x_ref,
            double t_ref,
            BasisValues& values) const
        {
            detail::zero_array_prefix(values);

            SpatialValues spatial_values{};
            evaluate_spatial_basis_on_patch_cell(
                patch_cell_index,
                x_ref,
                spatial_values);
            ...
            for (int i_space = 0; i_space < n_spatial_dofs(); ++i_space)
            {
                for (int i_time = 0; i_time < n_time_dofs(); ++i_time)
                {
                    ...
                    values[static_cast<std::size_t>(dof_id)] =
    ...
    private:
        void evaluate_spatial_on_cell_(
            int patch_cell_index,
            double x_ref,
            SpatialValues& values) const
        {
            detail::zero_array_prefix(values);
            (void)patch().cell(patch_cell_index);

            if (patch().is_boundary())
            {
            ...
            const auto legendre_values =
                detail::shifted_legendre_family<PSpace>(x_ref);
            const auto side = patch().cell(patch_cell_index).side;

            if (side == time_slabs::TimeSlabEdgePatchCellSide::left_of_vertex)
            {
                for (int degree = 1; degree <= PSpace; ++degree)
                {
            ...
            {
                const int offset = PSpace;
                for (int degree = 1; degree <= PSpace; ++degree)
                {
                    values[static_cast<std::size_t>(offset + degree - 1)] =
                        legendre_values[static_cast<std::size_t>(degree)];
                }
            }

            values[static_cast<std::size_t>(2 * PSpace)] =
                patch().weighted_haar_value(patch_cell_index);
        }
